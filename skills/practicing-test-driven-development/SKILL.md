---
name: superpowers-ccg-practicing-test-driven-development
...
**► Checkpoint 1 (Task Analysis):** Before writing test, apply checkpoint logic from `superpowers-ccg-coordinating-multi-model-work/checkpoints.md`:
...
**► Checkpoint 3 (Quality Gate):** After implementation passes, apply checkpoint logic from `superpowers-ccg-coordinating-multi-model-work/checkpoints.md`:
...
- Frontend component tests → GEMINI (`@frontend`)
- Backend logic tests → CODEX (`@backend`)
- Full-stack tests → CROSS_VALIDATION (call both)
...
**Fallback (Fail-Closed):** If external models are unavailable or time out, STOP and follow `superpowers-ccg-coordinating-multi-model-work/GATE.md` (do not proceed with generated tests).
