---
name: superpowers-ccg-brainstorming
...
**Model tip for exploration:** When dispatching subagents to explore the codebase, use `@explore` for fast, cost-effective searches. Haiku excels at file pattern matching and quick lookups.
...
**► Checkpoint 1 (Task Analysis):** After understanding the idea, apply checkpoint logic from `superpowers-ccg-coordinating-multi-model-work/checkpoints.md`:
...
**► Checkpoint 2 (Mid-Review):** When multiple approaches have significant trade-offs, apply checkpoint logic from `superpowers-ccg-coordinating-multi-model-work/checkpoints.md`:
...
- Use superpowers-ccg-using-git-worktrees to create isolated workspace
- Use superpowers-ccg-writing-plans to create detailed implementation plan
...
1. **Apply semantic routing** using `superpowers-ccg-coordinating-multi-model-work/routing-decision.md`
2. **Notify user**: "I will use [model] to evaluate this design"
3. **Call MCP tool** with English prompts (see `superpowers-ccg-coordinating-multi-model-work/INTEGRATION.md` for templates). Use `@backend` for backend, `@frontend` for frontend, and call both in parallel for CROSS_VALIDATION.
...
**Fallback (Fail-Closed):** If Codex MCP / Gemini MCP is unavailable or times out when Routing != CLAUDE, STOP and follow `superpowers-ccg-coordinating-multi-model-work/GATE.md` (do not proceed with a final design recommendation).
