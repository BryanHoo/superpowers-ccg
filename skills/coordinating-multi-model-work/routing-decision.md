...
Invoke this framework when a skill needs to call subagents (@backend, @frontend).
...
```
**Routing Decision:** [FRONTEND | BACKEND | CROSS_VALIDATION | CLAUDE]
**Rationale:** [One sentence]
```

**Early exposure note:** If `Routing != CLAUDE`, apply `superpowers-ccg-coordinating-multi-model-work/GATE.md` immediately (evidence or BLOCKED) before doing real work.

### Routing Targets

- **FRONTEND** - Frontend expert (@frontend) for UI, components, styles, interactions
- **BACKEND** - Backend expert (@backend) for APIs, databases, algorithms, server-side logic
- **CROSS_VALIDATION** - Both subagents (@backend + @frontend) for full-stack tasks, architectural decisions, or high uncertainty
- **CLAUDE** - Simple tasks that don't require specialized subagent output

## Decision Guidelines

- Strong frontend signals and weak/no backend signals → FRONTEND
- Strong backend signals and weak/no frontend signals → BACKEND
...
```
**Routing Decision:** CROSS_VALIDATION
**Rationale:** Authentication bug with unclear root cause requires multi-perspective diagnosis (@backend + @frontend)
```