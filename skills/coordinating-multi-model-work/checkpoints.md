...
- Use: semantic routing via `superpowers-ccg-coordinating-multi-model-work/routing-decision.md`

**Early exposure:** once you decide `Routing != CLAUDE`, immediately execute `GATE.md` (invoke subagents to obtain Evidence or output BLOCKED). Do not write plans or code first.
...
Goal: perform a final @backend/@frontend review before "final output/final conclusion/claiming tests passed/requesting code review".

- If `Routing != CLAUDE`: Evidence required
- Subagent failure: must BLOCKED (fail-closed)

## User Override

Users can explicitly override routing:

- "Use @backend" / "Use @frontend" / "Cross-validate" → force corresponding Routing
