---
name: superpowers-ccg-verifying-before-completion
...
**► Checkpoint 3 (Quality Gate):** Before claiming completion, apply checkpoint logic from `superpowers-ccg-coordinating-multi-model-work/checkpoints.md`:
...
At checkpoint, apply semantic routing using `superpowers-ccg-coordinating-multi-model-work/routing-decision.md`:

- Backend-only critical → CODEX verification (`@backend`)
- Frontend-only critical → GEMINI verification (`@frontend`)
- Full-stack/architectural → CROSS_VALIDATION (call both)
...
**Fallback (Fail-Closed):** If external models are unavailable or time out, STOP and follow `superpowers-ccg-coordinating-multi-model-work/GATE.md` (do not claim verification).
