---
name: superpowers-ccg-writing-plans
...
**► Checkpoint 1 (Task Analysis):** Before writing the plan, apply checkpoint logic from `superpowers-ccg-coordinating-multi-model-work/checkpoints.md`:
...
> **For Claude:** REQUIRED SUB-SKILL: Use superpowers-ccg-executing-plans to implement this plan task-by-task.
...
2. **Applies semantic routing** using `superpowers-ccg-coordinating-multi-model-work/routing-decision.md`:
...
3. Routes task to optimal model:
   ...
   - Full-stack/uncertain → CROSS_VALIDATION (call both)
...
**► Checkpoint 3 (Quality Gate):** Before handoff, apply checkpoint logic from `superpowers-ccg-coordinating-multi-model-work/checkpoints.md`:
...
- **REQUIRED SUB-SKILL:** Use superpowers-ccg-developing-with-subagents
...
- **REQUIRED SUB-SKILL:** New session uses superpowers-ccg-executing-plans
